import { InvalidEncriptedDataError, MinLengthError } from '@vnode/errors';
...
export class Encryption {
  ...
  private static validateKey(key: Buffer | string): void {
    ...
    if (keyBuffer.length !== this.KEY_LENGTH) {
      throw new MinLengthError(`Key must be ${this.KEY_LENGTH} bytes.`);
    }
  }
  ...
  public static parts(encryptedData: string): EncriptionParts {
    ...
    if (parts.length != 4) {
      throw new InvalidEncriptedDataError(`The encripted data is invalid.`);
    }
